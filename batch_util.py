...
def gen_sbatch(arg_array, sbatch_params, local=False, 
    ...
    with open(sbname, 'w') as sb:
        if local:
            if sbatch_params['cmd_args1'].get('serial', False):
                
                sb.write('#!/bin/bash\n')
                sb.write('export OMPI_MCA_shmem=^mmap\n')
                start_idx = sbatch_params['jobnos'][0]
                end_idx = sbatch_params['jobnos'][-1]
                sb.write('for i in {%d..%d}; do\n' % (start_idx, end_idx))
                sb.write('    python3 -u %s %s/arg${i}.dat --analysis_type=%s --resume --serial\n' %
                         (sbatch_params['script_path'], jobdir, sbatch_params['cmd_args0']['analysis_type']))
                sb.write('    wait\n')
                sb.write('done\n')
...
def init_batch(submit_file, jobdir, job_time='24:00:00', qos='regular', local=False, shifter=False, 
               sequential=False, resume=True, split_sbatch=False, serial=False, **kwargs):
    ...
    if hasattr(args, 'dim_filter'):
        dim_filter = args.dim_filter
    else:
        dim_filter = None
    ...
    if split_sbatch:
        ...
        for i, arg_array_ in enumerate(arg_array_split):
            sbatch_params = {'qos':qos, 'jobname':jobname, 'tpn': tpn,
                             ...
                             'cmd_args1': {'resume':resume, 'serial':serial}}
