...
def dicts_are_equal(dict1, dict2, keys=None):
    """
    Recursively check if two dictionaries are equal for shared keys.
    ...
    Returns:
        bool: True if dictionaries are equal for shared keys, False otherwise.
    """
    ...
    for key in shared_keys:
        ...
        if isinstance(value1, dict) and isinstance(value2, dict):
            if not dicts_are_equal(value1, value2):
                print(f"Dicts are not equal for key {key}")
                return False
        # Check if both values are lists
        elif isinstance(value1, list) and isinstance(value2, list):
            if len(value1) != len(value2) or any(not np.array_equal(np.array(v1), np.array(v2)) for v1, v2 in zip(value1, value2)):
                print(f"Lists are not equal for key {key}")
                return False
        # Check if both values are numpy arrays
        elif isinstance(value1, np.ndarray) and isinstance(value2, np.ndarray):
            if not np.array_equal(value1, value2):
                print(f"Arrays are not equal for key {key}")
                return False
        # Check for equality of other types
        elif value1 != value2:
            print(f"Values are not equal for key {key}")
            return False
