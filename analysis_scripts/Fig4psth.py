...
def PSTH_plot(top_neurons_df, region, data_path, session_key, df):
    ...
    for h, session in enumerate(sessions):
        ...
        x, time = get_rates_smoothed(data_path, region, session, return_t=True, trial_average=False, std=True, sigma=2, loader_args=loader_args, full_arg_tuple=fArgs)        
        ...
        for i in range(2):
            ...
            ax.text(1.0, 0.6, x_label_text, transform=ax.transAxes,
...
if __name__ == '__main__':
    ...
    for region in regions:    
        ...
        top_neurons_df, _ = get_top_neurons(df, session_key, dim_dict[region], fraction_cutoff=0.5, n=n_dict[region])
