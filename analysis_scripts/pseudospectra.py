    ...
    for region in regions:
        if not os.path.exists(PATH_DICT['tmp'] + f'/{region}_pseudo_tmp.pkl'):
            ...
            for i, session in tqdm(enumerate(sessions)):
                ...
                # Take into account trial structure
                if region == 'VISp':
                    xdiff_train = np.diff(xsmooth, axis=1)
                    # Chop off the last sample of each trial
                    xtrain = xsmooth[:, :-1]

                    xdiff_train = xdiff_train.reshape((-1, X.shape[-1]))
                    xtrain = xtrain.reshape((-1, X.shape[-1]))

                    xtrain = StandardScaler().fit_transform(xtrain)
                    xdiff_train = StandardScaler().fit_transform(xdiff_train)
                    linmodel = LinearRegression().fit(xtrain, xdiff_train)
                else:
        ...
        for i, session in enumerate(sessions):
            ...
            fig.savefig(PATH_DICT['figs'] + f'/{region}_pseudo/{session}.pdf', 
