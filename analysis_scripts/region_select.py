...
from loaders import (load_sabes, load_peanut, reach_segment_sabes,
                      ...
                      load_franklab_new,load_AllenVC, load_organoids)
...
loader_kwargs = {
    ...
    'organoids':{'load_idx':2, 'dec_idx':0, 'dr_idx':0},
}
...
def get_data_path(region):
    ...
    if region in ['M1', 'S1', 'M1_trialized', 'M1_psid', 'S1_psid']:
        ...
        data_path = 'AllenData'
    elif region == 'organoids':
        data_path = 'OrganoidData'
...
def load_decoding_df(region, **kwargs):
    ...
    if region == 'M1':
        ...
        path = '/home/marcush/Data/OrganoidData/neural_control_output/organoids_dimreduc'
        tmp_file = 'organoids_dimreduc_glom.pickle'
        with open(path + '/' + tmp_file, 'rb') as f:            
            rl = pickle.load(f)
        df = pd.DataFrame(rl)
        session_key = 'data_file'


        loader_args = [{'trial_length':1000,'bin_width':50, 'boxcox':0.5},
               {'trial_length':2000,'bin_width':50, 'boxcox':0.5},
               {'trial_length':2000,'bin_width':100, 'boxcox':0.5},
               {'trial_length':10000,'bin_width':50, 'boxcox':0.5},
               {'trial_length':10000,'bin_width':100, 'boxcox':0.5}]
        
        
        df = apply_df_filters(df, loader_args=loader_args[loader_kwargs[region]['load_idx']])
...
def load_data(data_path, region, session, loader_args, full_arg_tuple=None):
    ...
    elif region in ['organoids']:
        
        path_to_data = data_path + '/' + 'AllSessions.pkl'
        dat = load_organoids(path_to_data, **loader_args)
...
def get_rates_smoothed(data_path, region, session, trial_average=True,
    ...
    if region in ['M1', 'S1', 'M1_trialized']:
        ...
        loader_args = all_loader_args[loader_kwargs[region]['load_idx']]
